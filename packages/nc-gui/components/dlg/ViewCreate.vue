...
import type { FormType, GalleryType, GridType, KanbanType, PdfGeneratorType, TableType, ViewType } from 'nocodb-sdk'
...
interface Emits {
  ...
  (event: 'created', value: GridType | KanbanType | GalleryType | FormType | PdfGeneratorType): void
}
  ...
  // name is unique
  {
    validator: (_: unknown, v: string) =>
      new Promise((resolve, reject) => {
        views.every((v1) => ((v1 as GridType | KanbanType | GalleryType | PdfGeneratorType).alias || v1.title) !== v)
          ? resolve(true)
...
const typeAlias = computed(
  () =>
    ({
      ...
      [ViewTypes.PDF_GENERATOR_VIEW]: 'PdfGeneratorType',
    }[props.type]),
)
...
async function onSubmit() {
  ...
  if (isValid && form.type) {
    ...
    try {
      let data: GridType | KanbanType | GalleryType | FormType | PdfGeneratorType | null = null
      ...
      switch (form.type) {
        ...
        case ViewTypes.PDF_GENERATOR_VIEW:
          data = await api.dbView.pdfGeneratorCreate(_meta.id, form)
      }
