...
import { PdfGeneratorType, UITypes, ViewTypes } from 'nocodb-sdk';
...
console.log(
  'uiTypesToPdfTemplateTypesMapping',
  uiTypesToPdfTemplateTypesMapping
);
...
export async function pdfGeneratorViewGetExport(
  ...
  const templateSchema = Object.fromEntries(
    new Map(
      model.columns
        .filter((col) =>
          Object.keys(uiTypesToPdfTemplateTypesMapping).includes(col.uidt)
        )
        .map(
          (col, i) =>
            [
